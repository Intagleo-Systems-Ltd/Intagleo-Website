...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const study = await getCaseStudyBySlugAsync(slug);
  if (!study) return {};
    ...
    <main className="bg-[#0a0a0a] min-h-screen">
      ...
      <section className="section-padding pt-32 pb-12">
        <div className="mx-auto max-w-[1000px]">
          ...
          <h1 className="text-3xl md:text-5xl font-medium customHeading leading-tight mb-8">
            {study.title}
          </h1>
        </div>
      </section>
          ...
          {[
            ...
            <div
              ...
              className={`rounded-2xl p-6 border ${
                accent
                  ? "bg-[#3B82F6]/8 border-[#3B82F6]/20"
                  : "bg-[#0d0d10] border-white/8"
              }`}
            >
              <p className={`text-xs uppercase tracking-widest mb-3 font-semibold ${accent ? "text-[#3B82F6]" : "text-white/30"}`}>
                {label}
              </p>
